...
var flow_path = 'D:/accubits/Electron/PB/core/tagui';
...
chrome_step('Page.navigate',{url: 'https://www.amazon.in'}); sleep(1000);
techo('https://www.amazon.in' + ' - ' + chrome.getTitle() + '\n');});

casper.then(function() {{techo('click twotabsearchtextbox');
casper.waitFor(function check() {return check_tx('twotabsearchtextbox');},
function then() {chrome.click(tx('twotabsearchtextbox'));},
function timeout() {this.echo('ERROR - cannot find twotabsearchtextbox').exit();});}});

casper.then(function() {{techo('type twotabsearchtextbox as Leaving Time (with bonus novella Larger Than Life): A Novel');
casper.waitFor(function check() {return check_tx('twotabsearchtextbox');},
function then() {chrome.sendKeys(tx('twotabsearchtextbox'),'Leaving Time (with bonus novella Larger Than Life): A Novel');},
function timeout() {this.echo('ERROR - cannot find twotabsearchtextbox').exit();});}});

casper.then(function() {{techo('click nav-search-submit-button');
casper.waitFor(function check() {return check_tx('nav-search-submit-button');},
function then() {chrome.click(tx('nav-search-submit-button'));},
function timeout() {this.echo('ERROR - cannot find nav-search-submit-button').exit();});}});

casper.then(function() {{techo('click /html/body/div[1]/div[2]/div[1]/div[2]/div/span[3]/div[2]/div[3]/div/span/div/div/span/a/div/img');
casper.waitFor(function check() {return check_tx('/html/body/div[1]/div[2]/div[1]/div[2]/div/span[3]/div[2]/div[3]/div/span/div/div/span/a/div/img');},
function then() {chrome.click(tx('/html/body/div[1]/div[2]/div[1]/div[2]/div/span[3]/div[2]/div[3]/div/span/div/div/span/a/div/img'));},
function timeout() {this.echo('ERROR - cannot find /html/body/div[1]/div[2]/div[1]/div[2]/div/span[3]/div[2]/div[3]/div/span/div/div/span/a/div/img').exit();});}});

casper.then(function() {techo('popup Leaving-Time');});
chrome.waitForPopup(/Leaving\-Time/, function then() {},
function timeout() {this.echo('ERROR - cannot find popup Leaving-Time').exit();});
chrome.withPopup(/Leaving\-Time/, function() {
{ // start of code block

casper.then(function() {{techo('read //*[@id="productTitle"] to output');
casper.waitFor(function check() {return check_tx('//*[@id="productTitle"]');},
function then() {output = chrome.fetchText(tx('//*[@id="productTitle"]')).trim();},
function timeout() {this.echo('ERROR - cannot find //*[@id="productTitle"]').exit();});}});

casper.then(function() {{techo('dump '+output+' to output.txt');
save_text('D:/accubits/Electron/PB/core/tagui/output.txt',''+output+'');}});

casper.then(function() {{techo('type twotabsearchtextbox as inside new tab');
casper.waitFor(function check() {return check_tx('twotabsearchtextbox');},
function then() {chrome.sendKeys(tx('twotabsearchtextbox'),'inside new tab');},
function timeout() {this.echo('ERROR - cannot find twotabsearchtextbox').exit();});}});

casper.then(function() {{techo('click buy-now-button');
casper.waitFor(function check() {return check_tx('buy-now-button');},
function then() {chrome.click(tx('buy-now-button'));},
function timeout() {this.echo('ERROR - cannot find buy-now-button').exit();});}});

} // end of code block
});

casper.then(function() {techo('wait 20');});
casper.then(function() {casper.wait(20000, function() {});});
